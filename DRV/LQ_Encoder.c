...
int Read_Encoder(u8 TIMX)
{
  ...
  case 2:  Encoder_TIM= (short)TIM2 -> CNT;  TIM2 -> CNT=0;break;//编码器用
...
{    
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1, ENABLE);
  GPIO_PinAFConfig(GPIOE, GPIO_PinSource9|GPIO_PinSource11, GPIO_AF_1);//管脚复用AF1  
  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11|GPIO_Pin_9;//PE11 9
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//输入，悬空
  ...
  TIM_EncoderInterfaceConfig(TIM1, TIM_EncoderMode_TI12 , TIM_ICPolarity_Rising, TIM_ICPolarity_Rising);//使用编码器模式为TI12支持带龙-邱方向的编码器
