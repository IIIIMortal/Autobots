...
// 期望速度
volatile int16_t targetSpeed = 12;
//电机频率
...
void MotorInit(u16 freq)
{
  ...
  PWM_InitConfig(PWM8_CH3_PC8,freq, 1000);
  PWM_InitConfig(PWM8_CH4_PC9,freq, 1000);
...
{
  PWM_InitConfig(PWM5_CH1_PA0, 50, 1500);  //舵机默认用的是A0口，TIM5定时器
  PWM_InitConfig(PWM5_CH2_PA1, 50, 1500);  //舵机默认用的是A1口，TIM5定时器
}
