...
void MT9V034_Init(unsigned char fps)
{
  ...
  if(SCCB_RegRead(MT9V034_I2C_ADDR>>1,MT9V034_CHIP_VERSION,&data) == 0)  //读取摄像头版本寄存器
  {
...
#else
      TFTSPI_P8X8Str(0,9,"LQ-V034 id error",u16RED,u16BLUE);
#endif
    ...
    {
#ifdef USEOLED
      OLED_P6x8Str(0, 0, "LQ-V034 id OK");
#else
      TFTSPI_P8X8Str(0,9,"LQ-V034 id OK",u16RED,u16BLUE);
#endif
    }
  }
  else
...
#else
    TFTSPI_P8X8Str(0,10,"LQ-V034 sccb error",u16RED,u16BLUE);
#endif    
  ...
  TFTSPI_P8X8Str(0,11,"LQ-V034 init OK",u16RED,u16BLUE);
  delayms(200);//测试用的
}
...
* @date     2019/7/26 星期五
*/
GPIO_InitTypeDef  ccdi2cGPIO_InitStructure;

void MT9V034_SCL_Out(void)   //配置输出作为SCL_Out
{
  RCC_AHBPeriphClockCmd(RCC_AHBENR_GPIOF, ENABLE);
  ccdi2cGPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;
  ccdi2cGPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  ccdi2cGPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOF, &ccdi2cGPIO_InitStructure);
}
void MT9V034_SDA_Out(void)   //配置作为输出作为SDA_Out
{
  RCC_AHBPeriphClockCmd(RCC_AHBENR_GPIOF, ENABLE);
  ccdi2cGPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;
  ccdi2cGPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  ccdi2cGPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOF, &ccdi2cGPIO_InitStructure);
}
void MT9V034_SDA_In(void)    //配置作为输入作为SDA_In
{
  ...
  ccdi2cGPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;
  ccdi2cGPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  ccdi2cGPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
  GPIO_Init(GPIOF, &ccdi2cGPIO_InitStructure);
}
void SCCB_Init(void)
{
